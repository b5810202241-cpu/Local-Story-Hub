...
### 2026-09-23 — เขียนเทสต์ความปลอดภัยเพิ่มอีก 2 ตัว (ContentEditRequests, CommunityContent) — พบ+แก้บั๊กจริงอีกรายการ

ผู้ใช้ขอให้เพิ่มเทสต์ความปลอดภัยสำหรับ `ContentEditRequests` และ `CommunityContent` (ต่อยอดจาก 2 ตัว
แรกที่ทำไว้ก่อนหน้า — LSHRequests) วิเคราะห์ก่อนเขียนพบว่าทั้งสอง collection มี read rule รูปแบบ
เดียวกับ `LSHRequests` เดิม (`request.auth != null` กว้างเกินไป) แต่ต่างกันตรงที่:

- **`ContentEditRequests`** มีข้อมูลส่วนตัวจริง (คำขอแก้ไขที่ยัง "รอพิจารณา"/"ไม่อนุมัติ") — เขียน
  `tests/security-cross-community-edit-request-blocked.spec.js` ทดสอบว่าชุมชน B อ่านคำขอของชุมชน A
  ไม่ได้ — **ไม่ผ่านจริง** (ชุมชน B อ่าน `proposedChanges` ของ A สำเร็จผ่าน `getDoc()` ตรงๆ) แก้
  `LSH/firestore.rules` ให้แคบลงเหลือเจ้าของ/teacher เท่านั้น เหมือนที่แก้ `LSHRequests` ไปก่อนหน้า —
  deploy แล้ว retest ผ่าน — ผลข้างเคียง: ต้องแก้ query ใน `community-request-edit.html` เพิ่ม
  `where('communityId',...)` ด้วย เพราะ list query เดิมพิสูจน์ rule ใหม่ไม่ได้ (Firestore ปฏิเสธทั้ง
  query ไม่ใช่แค่กรองผลลัพธ์) ทำให้ฟีเจอร์ "เช็คคำขอค้างก่อนส่งใหม่" ของเจ้าของพังไปด้วยชั่วคราวก่อนแก้
- **`CommunityContent`** ตรวจแล้วไม่มี state ส่วนตัวให้ทดสอบจริง (`create` บังคับ `status ==
  'เผยแพร่แล้ว'` เสมอ, `update` แก้ `status` ไม่ได้เลย — ไม่มีทางสร้างเอกสารสถานะอื่นผ่านแอปได้) จึง
  เขียน `tests/security-community-content-no-draft-state.spec.js` เป็นเทสต์ล็อก invariant แทน (ยืนยัน
  ว่า `create` เอกสารสถานะอื่นต้องถูกปฏิเสธเสมอ) — **ผ่าน** ไม่ต้องแก้ rule ตอนนี้ แต่มีทริปไวร์ไว้ถ้า
  วันไหนมีคนเพิ่ม path ให้สร้าง draft ได้จริง

ระหว่างเขียน/รันเทสต์เจอ flaky test setup ไม่น้อย (ไม่เกี่ยวกับความถูกต้องของ rule แต่เกี่ยวกับ
Firestore listener/query ที่ค้าง "from cache" นานผิดปกติหลัง approve บัญชีชุมชน) แก้โดยเปลี่ยนไปเช็ค
ด้วย `getDoc()` ตรงๆ แทนการอ่านพฤติกรรม UI/query — ดูรายละเอียดเต็มที่ CLAUDE.md § เทสต์ความปลอดภัย
อัตโนมัติ — สรุปรวมตอนนี้มีเทสต์ความปลอดภัยถาวร 4 ตัว พบ+แก้บั๊กจริงไปแล้ว 2 รายการ (LSHRequests,
ContentEditRequests) เหลือแค่ที่ยืนยันแล้วว่ายังปลอดภัยจริง (CommunityContent) กับที่ยังไม่ได้ทดสอบ
เลย (ไม่มี — ครบทุก collection ที่มี pattern นี้แล้ว)

### 2026-09-23 — เขียนเทสต์ความปลอดภัยอัตโนมัติ 2 ตัวถาวร พบ+แก้บั๊กจริงใน firestore.rules

ผู้ใช้ขอให้เขียนเทสต์ความปลอดภัย 2 ตัวแบบถาวร (ไม่ใช่ QA manual ครั้งเดียว) ตามรูปแบบ "ห้ามตกข้อ" —
(1) ไม่ login แล้วเปิดหน้ารายการต้องอ่านไม่ได้ (2) login บัญชีที่สองแล้วเปิดทรัพยากรส่วนตัวของบัญชี
แรกต้องเปิดไม่ได้ — เสนอแผนให้ตรวจก่อน (พบจุดน่ากังวลระหว่างร่างแผน: `LSHRequests.read` rule เดิม
เปิดกว้างเกินไป) ผู้ใช้ confirm แผนแล้วสั่งเขียน+รัน

**ผลลัพธ์**: เขียน `tests/security-unauth-review-list-blocked.spec.js` และ
`tests/security-cross-user-pending-work-blocked.spec.js` (ถาวร ไม่ลบ) — รอบแรกทั้งสองไม่ผ่าน:

- ตัวแรก (unauth list) ไม่ผ่านเพราะ **เทสต์เขียนผิด** (`.card` selector กว้างเกินไป ไปแมตช์
  `#ai-summary-result` ที่เป็น `.card` เปล่า static อยู่ในหน้าเว็บอยู่แล้วไม่เกี่ยวกับข้อมูลรั่วเลย)
  แก้เทสต์ให้ชี้ container ที่มีข้อมูลจริงแทน (`#pending-list` ฯลฯ) → ผ่าน
- ตัวที่สอง (cross-user) ไม่ผ่านเพราะ **โค้ดผิดจริง** — `LSHRequests.read` rule เช็คแค่
  `request.auth != null` เปิดให้ผู้ใช้ login คนไหนก็ได้อ่านผลงานที่ยังไม่อนุมัติของคนอื่นได้หมด
  ยืนยันด้วย console ไม่มี permission-denied เลย (Firestore อนุญาตอ่านจริง ไม่ใช่แค่ UI ซ่อน) —
  ตามกฎที่ผู้ใช้ตั้งไว้ ("ห้ามแก้เทสต์เพื่อให้ผ่าน") จึงไปแก้ `LSH/firestore.rules` แทน ให้แคบลงเหลือ
  owner/teacher/public-approved เท่านั้น deploy ขึ้น production แล้ว retest ผ่านทั้งสองระดับ (UI +
  permission-denied จริง) ตรวจสอบเพิ่มว่าไม่กระทบฟีเจอร์เดิม (นิสิตอ่านผลงานตัวเอง, อาจารย์อ่านทุกอัน,
  public อ่านที่อนุมัติแล้ว) ผ่านครบ — ดูรายละเอียดที่ CLAUDE.md § "เทสต์ความปลอดภัยอัตโนมัติ"

**หมายเหตุ**: พบรูปแบบเดียวกัน (`request.auth != null` เปิดกว้างเกินไป) ใน `CommunityContent.read`
และ `ContentEditRequests.read` ด้วย แต่ **ยังไม่ได้แก้** เพราะยังไม่มีเทสต์ยืนยันว่ารั่วจริงในสโคปนี้
(สำหรับ `CommunityContent` สถานะปัจจุบันมีแค่ `เผยแพร่แล้ว` เท่านั้นจึงยังไม่กระทบจริง ส่วน
`ContentEditRequests` มีความเสี่ยงเดียวกัน) — ควรพิจารณาเพิ่มเทสต์/แก้ในรอบถัดไป

### 2026-09-23 — Deploy hosting + retest TC-006 หลัง regression พบว่า BL-010/BL-001 ยังไม่ขึ้น production

Regression test เต็มรูปแบบ (ดูหัวข้อถัดไปด้านล่าง) เจอ TC-006 ไม่ผ่าน — ตรวจสอบแล้วพบว่า**ไม่ใช่บั๊ก
ในโค้ด** โค้ด BL-010 (ปักหมุดตำแหน่ง) และ BL-001 (อัปโหลดภาพผ่าน R2) ที่ implement เสร็จและ commit
เข้า git แล้ว (`4e14352`, `b7dc605`) ไม่เคย deploy ขึ้น Firebase Hosting จริงเลย — `git commit`/
`git push` เป็นคนละขั้นตอนกับ `firebase deploy --only hosting` เช็คโค้ด production ยืนยันว่าไม่มี
`leaflet`/`location-map` อยู่จริง

รัน `firebase deploy --only hosting,firestore:rules` แล้ว ยืนยันด้วย `curl` ว่าโค้ดใหม่ขึ้น production
จริง จากนั้น retest TC-006 ซ้ำผ่าน browser จริง: login `u001@example.com` → ปักหมุดบนแผนที่ Leaflet
ใน `student-publish.html` → ส่งขออนุมัติ → login อาจารย์ (`u004@example.com`) อนุมัติ →
`tourist-story-detail.html` แสดง embedded pin map + ลิงก์ Google Maps ใช้พิกัดแม่นยำถูกต้องครบ —
**ผ่านสมบูรณ์บน production แล้ว** อัปเดตสถานะ TC-006 ใน test-plan.md เป็น "ผ่าน" พร้อมบันทึก root
cause ไว้ให้ชัดเจน (ดูรายละเอียดเต็มที่ [[../03-testing/02-test-result/index|02-test-result]])
...
พบบั๊ก 2 รายการระหว่างทาง (รายละเอียดเต็มใน [[../03-testing/02-test-result/20260923-test-run-tourist|20260923-test-run-tourist]]):

1. **[กลาง-สูง]** สมัครบัญชีนักท่องเที่ยวใหม่ครั้งแรกผ่านฟอร์ม "สมัครบัญชีใหม่" ที่ `tourist-login.html` — สร้างบัญชี Firebase Auth สำเร็จและ redirect กลับหน้าเดิมทันทีเหมือนสมัครสำเร็จ แต่เอกสาร `touristAccounts/{uid}` ไม่ถูกสร้างจริง (ไม่มี error แจ้งผู้ใช้เลย) ต้องกลับไปกรอกชื่อซ้ำผ่านการ์ด "ลงทะเบียนเป็นนักท่องเที่ยว" (complete-profile-view) อีกครั้งถึงจะใช้งานได้ — สงสัยว่าเป็น race condition ระหว่าง `onAuthStateChanged` กับ `await setDoc()` ใน `submitRegister()` ของ `tourist-login.html` (สังเกตเห็น `ERR_CONNECTION_RESET`/`ERR_INTERNET_DISCONNECTED` ใน console ช่วงเวลาใกล้เคียงกันด้วย อาจเป็นปัจจัยร่วม) ยังไม่ยืนยันสาเหตุ 100% — แนะนำให้ทีมพัฒนาตรวจ flow นี้
2. **[ต่ำ]** ไม่มีหน้ารวมรายการ "สถานที่โปรด" (My Bookmarks) ให้นักท่องเที่ยวดูภาพรวมทั้งหมดที่เคยบันทึกไว้ — มีแค่ toggle ต่อหน้ารายละเอียดเรื่องราวแต่ละหน้า (ตรวจโค้ดยืนยันว่าไม่มีหน้าอื่นอ่าน collection `Bookmarks` เลย) ไม่ตรงกับเจตนาเต็มรูปแบบของ AC BL-012 "ให้ดูภายหลังได้" แม้กลไกหลัก (บันทึก/ยกเลิก) จะทำงานถูกต้องและยืนยันด้วยการรีเฟรชแล้ว

อัปเดต [[../03-testing/01-test-plan/test-plan|test-plan]] คอลัมน์สถานะ/หมายเหตุของ TC-001–TC-008 ให้ตรงผลทดสอบจริงแล้ว — ข้อมูลทดสอบที่ทิ้งไว้ในระบบ: บัญชีนักท่องเที่ยว `qa-tester-tourist-lsh@example.com`, รีวิว 1 รายการ, บันทึกสถานที่โปรด 1 รายการ, ConsentRecord 1 รายการ (รายละเอียดครบใน test result ด้านบน)
...
### 2026-09-23 — ตรวจสอบและปรับปรุง UX ของ workaround BL-010 (วางแผนเส้นทางท่องเที่ยว)

รับมอบหมายให้ตรวจ BL-010 (Should priority) เฉพาะเจาะจง — `หมายเหตุ` เดิมของ backlog ระบุไว้แล้วว่า
ยังไม่มี field พิกัด/เส้นทางในสคีมา `LSHRequests`/`CommunityContent` เลย และไม่มีที่มาจาก requirement
ใดกำหนดรูปแบบไว้ จึงใช้ workaround ลิงก์ไป Google Maps ค้นหาด้วยชื่อชุมชนแทน — งานรอบนี้คือตรวจสอบว่า
workaround ยังถูกต้อง ปรับ UX ให้ชัดเจนขึ้น และขยายไปหน้าอื่นที่เข้าข่ายเดียวกัน **โดยไม่แตะ schema
หรือสร้างฟีเจอร์ปักหมุด/routing จริงตามที่ CLAUDE.md ห้ามเดา**

**ตรวจสอบ**: `tourist-story-detail.html` มีลิงก์ Google Maps อยู่แล้วจริง (`https://www.google.com/maps/search/` +
ชื่อชุมชนที่ encode แล้ว) ทดสอบผ่าน local static server + Playwright กับข้อมูลจริงใน Firestore project
`lsh-nammon` — href ออกมาถูกต้องทั้งกรณีผลงานนิสิต (`LSHRequests.community`) และคอนเทนต์ชุมชน
(`CommunityContent.communityName`) ไม่พบปัญหาเชิงฟังก์ชัน

**ปรับ UX**: ข้อความเดิมปนลิงก์ไว้กลางย่อหน้าอธิบายเหตุผล ("...ลองค้นหาชื่อชุมชนใน Google Maps ไปพลาง
ก่อน") ทำให้จุดที่กดได้ไม่เด่นชัด — แยกเป็นปุ่มลิงก์เฉพาะ "🗺️ ดูตำแหน่งบน Google Maps" ไว้ใต้คำอธิบาย
สั้นๆ ว่าทำไมยังปักหมุดในแอปไม่ได้ (`.map-box` ใน `tourist-story-detail.html`)

**ขยาย pattern เดิมไปอีก 2 หน้า** (ไม่มี field ใหม่ ไม่มี logic ใหม่ — ใช้สูตรเดียวกันคือ
`google.com/maps/search/` + ชื่อชุมชนที่มีอยู่แล้ว): `tourist-search-results.html` (เพิ่มลิงก์ต่อการ์ด
ผลการค้นหาแต่ละใบ ควบคู่กับลิงก์ "อ่านเรื่องราว →" เดิม) และ `published-works.html` (เพิ่มลิงก์ต่อการ์ด
ผลงาน เฉพาะรายการที่มี field `community` เท่านั้น — เอกสาร `LSHRequests` เก่าบางชิ้นไม่มี field นี้
จึงซ่อนลิงก์ให้แทนที่จะแสดง URL ค้นหาคำว่าง) ทดสอบผ่าน Playwright กับข้อมูลจริงทั้งสองหน้า ยืนยัน href
ถูกต้องและกรณีไม่มีชื่อชุมชนไม่ทำให้พัง

**Status ของ BL-010 ใน product-backlog.md ปรับจาก "ยังไม่เริ่ม" เป็น "ทำบางส่วน"** — สะท้อนความจริงว่า
มี interim workaround ใช้งานได้จริงครบ 3 หน้าแล้ว แต่ยังไม่ใช่ฟีเจอร์ปักหมุด/วางเส้นทางในแอปตาม
Acceptance Criteria ของ backlog

**สิ่งที่ต้องตัดสินใจ (ยังไม่ตอบเอง ตามกฎ CLAUDE.md ห้ามเดา schema/scope)** — บันทึกไว้ละเอียดใน
`หมายเหตุ` ของ BL-010 ใน `product-backlog.md` แล้ว สรุปสั้นๆ ที่นี่: (1) ชื่อ/รูปแบบ field พิกัดที่จะ
เพิ่มใน `LSHRequests`/`CommunityContent` — `GeoPoint` เดียวหรือแยก `lat`/`lng`, (2) ให้นิสิต/ชุมชนกรอก
พิกัดเองตอนสร้างเนื้อหา (ต้องเพิ่ม UI ปักหมุดในฟอร์ม) หรือ geocode อัตโนมัติจากชื่อสถานที่ (ต้องเลือก
ผู้ให้บริการ geocoding เพิ่ม), (3) ขอบเขตของ "หมุดหมายเดินทางที่ชัดเจน" ตาม Acceptance Criteria —
...
### 2026-09-23 — ผลทดสอบ TC-009 ถึง TC-016 (ชุมชน-คอนเทนต์)

ทดสอบผ่าน browser จริง (Playwright MCP) บน production (`https://lsh-nammon.web.app/`) เฉพาะหมวด
"ชุมชน — สร้าง/จัดการคอนเทนต์" ตามขอบเขตที่ได้รับมอบหมาย (ต่อจากรอบ TC-001–008 หมวดนักท่องเที่ยว) — ดู
[[../03-testing/02-test-result/20260923-test-run-community-content|20260923-test-run-community-content]]
สำหรับรายละเอียดเต็ม

**สรุป**: ทดสอบครบ 8/8 TC — ผ่าน 2 (TC-009 ระบบจัดการคอนเทนต์ของชุมชน/BL-006, TC-016 หน้าจอที่อ่านง่าย
สำหรับผู้สูงอายุ/BL-007), บล็อก 4 (TC-012–015 ทั้งหมดเพราะ Cloudflare Worker AI backend ยังไม่ deploy
จริง ตรงตามข้อจำกัดที่ทราบล่วงหน้า ไม่ใช่บั๊ก), ข้าม 2 (TC-010, TC-011 เพราะ BL-001 ยังไม่เริ่ม
implement — ไม่มีระบบอัปโหลดภาพจริง) **ไม่พบบั๊กใดๆ ในรอบทดสอบนี้**

**บัญชีทดสอบ**: บัญชีชุมชนเดิม "ชุมชนทดสอบผาบ่อง QA" จากรอบก่อนไม่มีรหัสผ่านบันทึกไว้ในเอกสารใดๆ เลย
(ตรวจสอบ `DEMO_CREDENTIALS.md` และ log แล้ว) จึงสมัครบัญชีชุมชนใหม่ผ่านฟอร์มจริง
("ทดสอบ QA ชุมชนบ้านผาบ่อง 2", `qa-tester-community-lsh@example.com`) แล้วอนุมัติเองด้วยบัญชีอาจารย์
ผ่าน `admin-review-student-work.html`, สร้าง/เผยแพร่คอนเทนต์ทดสอบ 1 รายการ (`CommunityContent`) ยืนยัน
คงอยู่จริงหลังรีเฟรชหน้า — เก็บข้อมูลทดสอบทั้งหมดไว้ในระบบตามธรรมเนียมเดิม (ไม่ลบทิ้ง)

**ข้อสังเกตสำหรับรอบถัดไป**: ควรพิจารณาบันทึก credential ของบัญชีทดสอบไว้ในที่ปลอดภัยที่ QA agent
รอบถัดไปเข้าถึงได้ เพื่อลดการสร้างบัญชีทดสอบซ้ำซ้อนขึ้นเรื่อยๆ ในแต่ละรอบทดสอบ

### 2026-09-23 — ผลทดสอบ TC-017 ถึง TC-019 (ชุมชน-สมัครบัญชี)

ทดสอบผ่าน browser จริง (Playwright MCP) บน production (`https://lsh-nammon.web.app/`) เฉพาะหมวด
"ชุมชน — สมัคร/อนุมัติบัญชีชุมชนใหม่" ตามขอบเขตที่ได้รับมอบหมาย (ต่อจากรอบ TC-009–016 หมวดชุมชน-
คอนเทนต์) ทดสอบเป็นชุดเดียวกันตามลำดับเหตุการณ์จริง (สมัคร → อนุมัติ → ปฏิเสธ) — ดู
[[../03-testing/02-test-result/20260923-test-run-community-registration|20260923-test-run-community-registration]]
สำหรับรายละเอียดเต็ม

**สรุป**: ทดสอบครบ 3/3 TC — **ผ่านทั้งหมด 3 ข้อ** (TC-017 สมัครบัญชี→สถานะรออนุมัติ+แจ้งเตือนอาจารย์,
TC-018 อนุมัติแล้วใช้ฟีเจอร์หลักได้จริง ยืนยันด้วยการสร้าง/เผยแพร่คอนเทนต์จริงสำเร็จ, TC-019 ปฏิเสธแล้ว
login ไม่ได้พร้อมแจ้งเหตุผลถูกต้อง) ไม่พบบั๊กที่ทำให้ Acceptance Criteria ไม่เป็นจริง — พบข้อสังเกต
ความรุนแรงต่ำ 1 รายการ: บัญชีที่ถูกปฏิเสธยังมี Firebase Auth session ค้างอยู่ (ไม่ถูก sign-out
อัตโนมัติ) เมื่อ navigate ตรงไปหน้าอื่น (เช่น `community-dashboard.html`) นอกเหนือจากหน้า login
ระบบบล็อกการเข้าถึงถูกต้อง แต่ข้อความที่แสดงคือ "บัญชีนี้ยังอยู่ในสถานะรออนุมัติ" (ข้อความของสถานะ
"รออนุมัติ") แทนที่จะเป็นข้อความเฉพาะของสถานะ "ไม่อนุมัติ" พร้อมเหตุผลแบบที่หน้า login แสดงถูกต้อง —
ไม่กระทบความปลอดภัย/ผลทดสอบหลัก แนะนำให้ทีมพัฒนาตรวจ guard function ให้ครบทั้ง 3 สถานะ

**บัญชีทดสอบ**: สมัครบัญชีชุมชนทดสอบใหม่ 2 บัญชีผ่านฟอร์มจริงตามที่ได้รับมอบหมาย —
"ทดสอบ QA ชุมชนอนุมัติ" (`qa-community-approve-lsh@example.com`, อนุมัติแล้ว, สร้างคอนเทนต์ทดสอบ 1
รายการเพื่อยืนยัน TC-018) และ "ทดสอบ QA ชุมชนปฏิเสธ" (`qa-community-reject-lsh@example.com`, ไม่อนุมัติ
— สถานะสิ้นสุดตาม Business Rule BL-022) เก็บข้อมูลทดสอบทั้งหมดไว้ในระบบตามธรรมเนียมเดิม (ไม่ลบทิ้ง)

### 2026-09-23 — ผลทดสอบ TC-020 ถึง TC-022 (ชุมชน-ขอแก้ไข)

ทดสอบผ่าน browser จริง (Playwright MCP) บน production (`https://lsh-nammon.web.app/`) เฉพาะหมวด
"ชุมชน — ขอแก้ไขข้อมูล/คอนเทนต์" ตามขอบเขตที่ได้รับมอบหมาย (ต่อจากรอบ TC-017–019 หมวดชุมชน-สมัคร
บัญชี) ทดสอบทั้งเส้นทางอนุมัติและไม่อนุมัติคำขอแก้ไข โดยส่งคำขอแก้ไข 2 ครั้งแยกกันบนคอนเทนต์เดียวกัน
(ครั้งที่ 1 ให้อนุมัติ, ครั้งที่ 2 ให้ไม่อนุมัติ) — ดู
[[../03-testing/02-test-result/20260923-test-run-community-edit-request|20260923-test-run-community-edit-request]]
สำหรับรายละเอียดเต็ม

**สรุป**: ทดสอบครบ 3/3 TC — **ผ่าน 2 ข้อ** (TC-020 ส่งคำขอแก้ไข→สถานะ "รอพิจารณา" คอนเทนต์จริงยังไม่
เปลี่ยน, TC-021 อาจารย์อนุมัติแล้วคอนเทนต์จริงเปลี่ยนทันที ยืนยันทั้งฝั่งจัดการของชุมชนและหน้าสาธารณะ),
**ไม่ผ่าน 1 ข้อ (TC-022) — พบบั๊กความรุนแรงปานกลาง**: ครึ่งแรกของ Then ("ระบบไม่นำการแก้ไขไปใช้")
ทำงานถูกต้อง (ยืนยันคอนเทนต์จริงไม่เปลี่ยนหลังปฏิเสธ) แต่ครึ่งหลัง ("และแจ้งเหตุผลกลับไปยังชุมชน")
**ไม่ทำงานเลย** — ตรวจ source code ยืนยันว่า `community-dashboard.html` query `ContentEditRequests`
เฉพาะ `status == 'รอพิจารณา'` เท่านั้น ไม่เคยอ่าน/แสดงคำขอที่ถูกปฏิเสธหรือ field `rejectionReason` เลย
ทั้ง 3 ไฟล์ที่แตะ collection นี้ (`admin-review-student-work.html`, `community-dashboard.html`,
`community-request-edit.html`) เหตุผลที่อาจารย์กรอกไว้ถูกเก็บและแสดงถูกต้องเฉพาะฝั่งอาจารย์เท่านั้น —
ฝั่งชุมชนเห็นแค่ปุ่ม "ขอแก้ไข" กลับมาใช้งานได้ตามปกติโดยไม่มีคำอธิบายใดๆ แยกไม่ออกจากกรณี "ไม่เคยส่งคำขอ"
— ไม่กระทบความปลอดภัย/ความถูกต้องของข้อมูล แต่ขัดกับ Acceptance Criteria ของ BL-023 ที่ระบุไว้ชัดเจน
สถานะ backlog เดิม (`เสร็จแล้ว`) จึงไม่ตรงกับพฤติกรรมจริงทั้งหมด แนะนำให้ทีมพัฒนาเพิ่ม UI ฝั่งชุมชนแสดง
คำขอที่พิจารณาแล้วพร้อมเหตุผลก่อนปิดสถานะสมบูรณ์

**บัญชีทดสอบ**: ใช้บัญชีชุมชนเดิมจากรอบก่อน `qa-community-approve-lsh@example.com` (login สำเร็จ ไม่ต้อง
สมัครใหม่) และคอนเทนต์เดิม "ทดสอบ QA - ทดสอบสิทธิ์ชุมชนอนุมัติ" (`documentId: 2DejQRLf0oRwlNgFAXZF`) —
ชื่อ/เนื้อหาของคอนเทนต์นี้ถูกแก้ไขจริงถาวรระหว่างทดสอบ (ผ่านคำขอที่ได้รับอนุมัติ) เป็น "ทดสอบ QA -
ทดสอบสิทธิ์ชุมชนอนุมัติ (แก้ไข TC-021 อนุมัติ)" เก็บคำขอแก้ไขทั้ง 2 รายการ (อนุมัติ 1, ไม่อนุมัติ 1)
ไว้ในระบบเป็นหลักฐานการทดสอบตามธรรมเนียมเดิม (ไม่ลบทิ้ง) — ก่อนเริ่มทดสอบพบ Firebase Auth session ค้าง
จากรอบก่อนในเบราว์เซอร์ (บัญชีชุมชนที่ถูกปฏิเสธ) แก้ไขด้วยการ navigate ไปหน้า `admin-review-student-work.html`
ซึ่งมีปุ่ม "ออกจากระบบ" ที่มองเห็นได้เสมอเมื่อ login อยู่ (ไม่ว่า role ใด) แล้วกด logout ผ่าน UI จริงก่อนเริ่ม
ทดสอบจริง — ไม่ได้ใช้ JS injection

### 2026-09-23 — ผลทดสอบ TC-023 ถึง TC-026 (นิสิต-คอนเทนต์) — ยืนยัน dropdown ชุมชน BL-013 จบ end-to-end แล้ว

ทดสอบผ่าน browser จริง (Playwright MCP) บน production (`https://lsh-nammon.web.app/`) เฉพาะหมวด
"นิสิตนิเทศศาสตร์" ตามขอบเขตที่ได้รับมอบหมาย (ต่อจากรอบ TC-020–022 หมวดชุมชน-ขอแก้ไข) จุดเน้นพิเศษของ
รอบนี้คือการ retest dropdown "เลือกชุมชนที่เกี่ยวข้อง" ใน `student-publish.html` (BL-013) ที่มีบันทึกค้าง
ไว้ตั้งแต่รอบก่อนหน้าว่ายังไม่เคย retest จริงผ่านเบราว์เซอร์หลัง deploy `firestore.rules` — ดู
[[../03-testing/02-test-result/20260923-test-run-student-content|20260923-test-run-student-content]]
สำหรับรายละเอียดเต็ม

**ผลการตรวจ dropdown ชุมชน BL-013 (จุดเน้นพิเศษ)**: **ผ่านครบทั้ง 3 ข้อที่ต้องตรวจ — ยืนยันจบ
end-to-end แล้ว** (ก) login ด้วยบัญชีนิสิตจริง dropdown โหลดรายชื่อชุมชนจริงจาก Firestore ได้ทันที ตรวจ
console messages (ทุก level) และ network requests ครบทุก request ที่เกี่ยวกับ `users`/`LSHRequests` **ไม่
พบ error "Missing or insufficient permissions" เลย** ยืนยันว่า OR clause ที่เพิ่มใน `firestore.rules`
(ให้นิสิต query บัญชีชุมชน `role=='community' && status=='อนุมัติแล้ว'` ได้) ทำงานถูกต้องหลัง deploy (ข)
รายชื่อที่โชว์ตรงกับบัญชีชุมชนที่อนุมัติแล้วจริงครบทั้ง 3 รายการ (`ชุมชนทดสอบผาบ่อง QA`, `ทดสอบ QA
ชุมชนบ้านผาบ่อง 2`, `ทดสอบ QA ชุมชนอนุมัติ`) ไม่ขาดไม่เกิน (ค) เลือกชุมชนแล้วส่งผลงานได้จริง ชุมชนที่เลือก
ถูกบันทึกลง `LSHRequests` และแสดงถูกต้องตรงกันทุกจุด: หน้าอาจารย์ (การ์ดรออนุมัติ+ตารางประวัติ), หน้า
สาธารณะ `published-works.html`, และหน้า `tourist-search-results.html` — ตรวจซ้ำหลัง refresh ทุกจุดที่มี
การเขียนข้อมูล ผลลัพธ์คงเดิมทุกครั้ง **BL-013 ถือว่าเสร็จสมบูรณ์เต็มรูปแบบแล้วจากมุมมองการทดสอบนี้ ปิด
ช่องว่างที่ค้างไว้ตั้งแต่รอบก่อนหน้าได้สำเร็จ**

**สรุปผล TC**: ทดสอบครบ 4/4 TC — **ผ่าน 3 ข้อ** (TC-023 ส่งผลงาน→สถานะรออนุมัติยังไม่เผยแพร่, TC-024
เชื่อมโยงผลงานกับชุมชนถูกต้อง/dropdown BL-013 ผ่านครบ, TC-025 อาจารย์อนุมัติแล้วเผยแพร่ทันที),
**ไม่ผ่าน 1 ข้อ (TC-026) — พบบั๊กความรุนแรงปานกลาง**: ครึ่งแรกของ Then ("ระบบไม่เผยแพร่ผลงานนั้น")
ทำงานถูกต้อง แต่ครึ่งหลัง ("และแจ้งเหตุผลกลับไปยังนิสิต") **ไม่ทำงานเลย** — ตรวจ source code ยืนยันว่า
`student-publish.html` ไม่มีจุดใดเลยที่ query กลับมาอ่านผลงานเดิมของนิสิตคนนั้นจาก `LSHRequests` (ไม่มี
`where('requesterId','==', currentProfile.id)`) ส่วนที่อ่าน `rejectionReason` กลับมาแสดง (บรรทัด 279-280)
เป็นของบัญชีผู้ใช้ (`users`) เท่านั้น (กรณีบัญชีถูกปฏิเสธตอนสมัคร ไม่ใช่กรณีผลงานถูกปฏิเสธ) — เหตุผลที่
อาจารย์กรอกไว้ถูกเก็บและแสดงถูกต้องเฉพาะฝั่งอาจารย์เท่านั้น **เป็นรูปแบบบั๊กเดียวกันเป๊ะกับที่พบใน TC-022
(ชุมชน-ขอแก้ไขคอนเทนต์)** — ทั้งสองจุดขาด UI/query ฝั่งผู้ส่งคำขอที่แสดงประวัติ+เหตุผลของคำขอที่ถูกปฏิเสธ
แนะนำให้ทีมพัฒนาแก้พร้อมกันทั้งสองจุดด้วยรูปแบบเดียวกัน ไม่กระทบความปลอดภัย/ความถูกต้องของข้อมูล แต่ขัดกับ
Acceptance Criteria ของ BL-018 ที่ระบุไว้ชัดเจน

**บัญชีทดสอบ**: ใช้บัญชีนิสิต/อาจารย์เดิมจาก `LSH/DEMO_CREDENTIALS.md` ทั้งหมด (`u001@example.com`,
`u002@example.com`, `u004@example.com`) ไม่ต้องสมัครใหม่ สร้างผลงานทดสอบใหม่ 2 รายการใน `LSHRequests`
("ทดสอบ QA - ผลงานนิสิต TC-023 ถึง TC-026" สถานะอนุมัติ, "ทดสอบ QA - ผลงานนิสิต TC-026 (จะถูกไม่อนุมัติ)"
สถานะไม่อนุมัติ) เก็บไว้ในระบบเป็นหลักฐานการทดสอบตามธรรมเนียมเดิม (ไม่ลบทิ้ง) — ก่อนเริ่มทดสอบพบ Firebase
Auth session ค้างจากรอบก่อน (บัญชีชุมชน) แก้ไขด้วยการ logout ผ่านปุ่ม "ออกจากระบบ" บนหน้า
`admin-review-student-work.html` (มองเห็นได้เสมอไม่ว่า role ใด) ตามปกติ ไม่ได้ใช้ JS injection

### 2026-09-23 — ผลทดสอบ TC-027 ถึง TC-032 (นิสิต-สมัครบัญชี)

ทดสอบผ่าน browser จริง (Playwright MCP) บน production (`https://lsh-nammon.web.app/`) เฉพาะหมวด
"นิสิต — สมัคร/อนุมัติบัญชีผู้ใช้ใหม่" ตามขอบเขตที่ได้รับมอบหมาย (ต่อจากรอบ TC-023–026 หมวดนิสิต-คอนเทนต์)
สมัครบัญชีนิสิตทดสอบใหม่ 2 บัญชีจริงผ่าน `student-publish.html` (1 บัญชีให้อนุมัติ, 1 บัญชีให้ไม่อนุมัติ)
แล้วดำเนินการอนุมัติ/ปฏิเสธด้วยบัญชีอาจารย์ `u004@example.com` ผ่าน `admin-review-student-work.html` — ดู
[[../03-testing/02-test-result/20260923-test-run-student-registration|20260923-test-run-student-registration]]
สำหรับรายละเอียดเต็ม

**จุดเน้นพิเศษของรอบนี้ — ตรวจข้อความแจ้งบัญชีถูกปฏิเสธ (TC-029/TC-032) อย่างละเอียด**: เพราะ 2 รอบก่อน
พบบั๊กรูปแบบเดียวกันซ้ำ 2 ครั้ง ("ฝั่งผู้ยื่นคำขอไม่เคยเห็นสถานะ/เหตุผลตอนถูกปฏิเสธ" — TC-022 คำขอแก้ไข
ชุมชน, TC-026 ผลงานนิสิต) จึงทดสอบ TC-029/TC-032 ซ้ำหลายรูปแบบ (fresh login ใหม่หลังถูกปฏิเสธ, รีเฟรชหน้า,
navigate ออกไปหน้าอื่นแล้วกลับมา) — **ผลคือไม่พบรูปแบบบั๊กเดียวกัน** `student-publish.html` แสดงข้อความ
"อาจารย์ที่ปรึกษาไม่อนุมัติบัญชีนี้ — เหตุผล: [เหตุผลที่อาจารย์กรอกไว้เป๊ะๆ]" พร้อม "บัญชีนี้ใช้งานต่อไม่ได้
— หากต้องการเข้าใช้งานต้องสมัครสมาชิกใหม่เท่านั้น" ถูกต้องและสม่ำเสมอทุกครั้งที่ทดสอบซ้ำ ต่างจากฝั่งชุมชน
(TC-019 รอบก่อน) ที่เคยพบข้อสังเกตความรุนแรงต่ำเรื่องข้อความไม่สอดคล้องกันเมื่อ navigate ตรงไปหน้าอื่น —
สันนิษฐานว่าเป็นเพราะ `student-publish.html` รวมหน้า login และหน้าแสดงสถานะบัญชีไว้ในไฟล์เดียวกัน ไม่ต้อง
guard สถานะซ้ำในหลายไฟล์แบบฝั่งชุมชน

**สรุปผล TC**: ทดสอบครบ 6/6 TC — **ผ่าน 5 ข้อ** (TC-027 สมัครบัญชีสำเร็จสถานะรออนุมัติทันที, TC-028
บัญชีรออนุมัติ login ไม่ได้พร้อมข้อความถูกต้อง, TC-029 บัญชีถูกปฏิเสธ login ไม่ได้พร้อมเหตุผลถูกต้อง,
TC-031 อนุมัติแล้ว login ใช้งานฟีเจอร์หลักได้จริง ยืนยันด้วยการส่งผลงานสำเร็จ, TC-032 ไม่อนุมัติพร้อมเหตุผล
ครบทั้ง 3 ส่วนของ Then), **บล็อก 1 ข้อ (TC-030)** — ทดสอบอีเมลแจ้งเตือนอาจารย์จริงผ่านเบราว์เซอร์ไม่ได้ (ไม่มี
กล่องอีเมลให้เช็ค ตรงกับที่ทราบอยู่แล้วว่ายังไม่มี Notification Service จริง) แต่ตรวจสอบและทดสอบกลไกทดแทน
สำเร็จ — พบแบนเนอร์ "🔔 มีรายการรอพิจารณา..." ที่หน้าอาจารย์แสดงจำนวนบัญชีใหม่รออนุมัติถูกต้องทันทีที่ login
ไม่นับเป็นบั๊กเพราะเป็นข้อจำกัดที่ตัดสินใจข้ามไปก่อนแล้ว **ไม่พบบั๊กใหม่ในขอบเขตรอบนี้เลย**

**บัญชีทดสอบ**: สมัครบัญชีนิสิตทดสอบใหม่ 2 บัญชีผ่านฟอร์มจริง — `qa-tester-student-approve-lsh@example.com`
(อนุมัติแล้ว, ส่งผลงานทดสอบ 1 รายการสถานะรอพิจารณา) และ `qa-tester-student-reject-lsh@example.com`
(ไม่อนุมัติ, เหตุผล "ทดสอบ QA: ข้อมูลในผลงานไม่ครบถ้วน") เก็บไว้ในระบบเป็นหลักฐานการทดสอบตามธรรมเนียมเดิม
(ไม่ลบทิ้ง) ไม่มี console error ตลอดการทดสอบ ไม่ได้ใช้ JS injection

### 2026-09-23 — ผลทดสอบ TC-033 ถึง TC-036 (บุคคลทั่วไป) — ปิดรอบทดสอบครบ 36/36 ข้อ

QA agent ทดสอบผ่านเบราว์เซอร์จริง (Playwright MCP) กับเว็บ production (`https://lsh-nammon.web.app/`)
เฉพาะขอบเขต TC-033–TC-036 (หมวด "บุคคลทั่วไป (ไม่ Login)") ใน
[[../03-testing/01-test-plan/test-plan|test-plan]] — **นี่คือรอบทดสอบสุดท้ายในชุดนี้ ปิด test plan
ให้ครบทั้ง 36/36 ข้อแล้ว** (ต่อจาก 6 รอบก่อนหน้าที่ทดสอบ TC-001–032) ดู
[[../03-testing/02-test-result/20260923-test-run-public-view|20260923-test-run-public-view]] สำหรับ
รายละเอียดเต็ม

**ผลลัพธ์รอบนี้**: ทดสอบครบ 4/4 TC — **ผ่านทั้งหมด ไม่พบบั๊กเลย**

**จุดเน้นพิเศษ — TC-035 (security test เรื่องการเข้าถึงผลงานที่ยังไม่อนุมัติ)**: เตรียมข้อมูลทดสอบเอง
ผ่าน UI ปกติ (login ชั่วคราวด้วยบัญชีนิสิต/อาจารย์เพื่อสร้างผลงานทดสอบแล้วปฏิเสธ ให้ได้ document ID จริง
คือ `74eCxoxJ2NXoCN8gTk2Q` ในสถานะ "ไม่อนุมัติ") แล้ว logout ทดสอบเป็นบุคคลทั่วไปจริงทั้ง 3 ช่องทางตามที่
ได้รับมอบหมาย ครบทั้ง 2 สถานะ ("รอพิจารณา"/"ไม่อนุมัติ"): (ก) เข้าถึงตรงผ่าน URL
(`tourist-story-detail.html?type=student&id=...`) ด้วย document ID จริง → ได้ error "Missing or
insufficient permissions" เท่านั้น ไม่มีเนื้อหาหลุดออกมาเลย (ข) ค้นหาชื่อผลงานทดสอบในทุกหน้าค้นหา public
(`published-works.html`, `tourist-search-results.html`) → ไม่พบผลลัพธ์เลยสักครั้ง (ค) ตรวจ Firestore
query จริงที่ยิงออกไปผ่าน `browser_network_request` (request body ของ Listen channel) → ยืนยันว่าทุก
query กรอง `status == 'อนุมัติ'` (หรือ `'เผยแพร่แล้ว'` สำหรับ `CommunityContent`) **ที่ฝั่ง server ตั้งแต่
ต้น** ไม่ใช่รูปแบบ fetch-all-then-filter-on-client ที่จะทำให้ข้อมูลรั่วได้ — **ไม่พบข้อมูลรั่วไหลแม้แต่
นิดเดียวในทุกช่องทางที่ทดสอบ** ยืนยันว่า `firestore.rules` บังคับสิทธิ์ทั้งระดับ query และระดับ document
read ถูกต้อง

**บัญชี/ข้อมูลทดสอบ**: ใช้บัญชีนิสิต/อาจารย์เดิม (`u001@example.com`, `u004@example.com`) เฉพาะขั้นตอน
เตรียมข้อมูล ไม่ได้ใช้ทดสอบ TC จริง — สร้างผลงานทดสอบใหม่ 1 รายการ ("ทดสอบ QA - ผลงานนิสิต TC-035
(รอพิจารณา ห้ามเข้าถึงแบบ public)", document ID `74eCxoxJ2NXoCN8gTk2Q`, สถานะสุดท้าย "ไม่อนุมัติ") เก็บ
ไว้ในระบบเป็นหลักฐานการทดสอบ ไม่ได้ใช้ JS injection ตลอดการทดสอบ

---

## สรุปภาพรวมการทดสอบทั้ง 7 รอบ (TC-001 ถึง TC-036) — ปิดรอบทดสอบ QA ครบ 36/36 ข้อ

ทดสอบผ่านเบราว์เซอร์จริง (Playwright MCP) บน production (`https://lsh-nammon.web.app/`) ครบทั้ง 36
test case ใน [[../03-testing/01-test-plan/test-plan|test-plan]] แบ่งเป็น 7 รอบตามหมวด journey:

| รอบ | ขอบเขต | ผ่าน | ไม่ผ่าน | บล็อก | ข้าม |
|---|---|---|---|---|---|
| 1 — นักท่องเที่ยว | TC-001–008 | 6 | 0 | 2 | 0 |
| 2 — ชุมชน-คอนเทนต์ | TC-009–016 | 2 | 0 | 4 | 2 |
| 3 — ชุมชน-สมัครบัญชี | TC-017–019 | 3 | 0 | 0 | 0 |
| 4 — ชุมชน-ขอแก้ไข | TC-020–022 | 2 | 1 | 0 | 0 |
| 5 — นิสิต-คอนเทนต์ | TC-023–026 | 3 | 1 | 0 | 0 |
| 6 — นิสิต-สมัครบัญชี | TC-027–032 | 5 | 0 | 1 | 0 |
| 7 — บุคคลทั่วไป | TC-033–036 | 4 | 0 | 0 | 0 |
| **รวม** | **36 ข้อ** | **25** | **2** | **7** | **2** |

**บล็อก 7 ข้อ ทุกข้อบล็อกเพราะข้อจำกัดที่ทราบล่วงหน้าอยู่แล้ว ไม่ใช่บั๊กใหม่**: TC-003 (consent banner
ตอบได้ครั้งเดียวต่อเบราว์เซอร์ ทดสอบซ้ำไม่ได้โดยไม่ใช้ JS injection), TC-005/012/013/014/015 (Cloudflare
Worker AI backend proxy ยังไม่ได้ deploy จริง), TC-030 (ไม่มี Notification Service ส่งอีเมลจริงให้ทดสอบ) —
**ข้าม 2 ข้อ**: TC-010/011 (BL-001 AI ปรับภาพ ยังไม่เริ่ม implement เพราะไม่มีระบบอัปโหลดภาพจริง)

### รายชื่อบั๊กทั้งหมดที่พบตลอดการทดสอบทั้ง 7 รอบ (เรียงตามความรุนแรง)

1. **[กลาง-สูง] สมัครบัญชีนักท่องเที่ยวใหม่ครั้งแรกไม่สำเร็จเงียบๆ (TC-007, รอบ 1)** — สมัครบัญชีผ่าน
   ฟอร์ม "สมัครบัญชีใหม่" ที่ `tourist-login.html` สร้างบัญชี Firebase Auth สำเร็จและ redirect เหมือน
   สมัครสำเร็จ แต่เอกสาร `touristAccounts/{uid}` ไม่ถูกสร้างจริง ไม่มี error แจ้งผู้ใช้เลย ต้องกรอกชื่อซ้ำ
   ผ่านการ์ด "ลงทะเบียนเป็นนักท่องเที่ยว" อีกครั้งถึงจะใช้งานได้ — สงสัยว่าเป็น race condition ระหว่าง
   `onAuthStateChanged` กับ `await setDoc()` ยังไม่ยืนยันสาเหตุ 100% — ดู
   [[../03-testing/02-test-result/20260923-test-run-tourist|20260923-test-run-tourist]]
2. **[ปานกลาง] ไม่แจ้งเหตุผลการไม่อนุมัติคำขอแก้ไขคอนเทนต์กลับไปยังชุมชนเลย (TC-022, รอบ 4)** —
   `community-dashboard.html` query `ContentEditRequests` เฉพาะ `status == 'รอพิจารณา'` เท่านั้น ไม่เคย
   อ่าน/แสดงคำขอที่ถูกปฏิเสธหรือ `rejectionReason` เลย ฝั่งชุมชนเห็นแค่ปุ่ม "ขอแก้ไข" กลับมาใช้งานได้ตาม
   ปกติโดยไม่มีคำอธิบายใดๆ แยกไม่ออกจากกรณี "ไม่เคยส่งคำขอ" — ดู
   [[../03-testing/02-test-result/20260923-test-run-community-edit-request|20260923-test-run-community-edit-request]]
3. **[ปานกลาง] ไม่แจ้งเหตุผลการไม่อนุมัติผลงานกลับไปยังนิสิตเลย (TC-026, รอบ 5)** — รูปแบบบั๊กเดียวกัน
   เป๊ะกับข้อ 2 แต่ฝั่งนิสิต: `student-publish.html` ไม่มีจุดใดเลยที่ query กลับมาอ่านผลงานเดิมของนิสิต
   คนนั้นจาก `LSHRequests` (ไม่มี `where('requesterId','==', currentProfile.id)`) นิสิตที่ถูกปฏิเสธผลงาน
   จะไม่รู้เลยว่าผลงานถูกปฏิเสธหรือทำไม นอกจากไปถามอาจารย์โดยตรงนอกระบบ — ดู
   [[../03-testing/02-test-result/20260923-test-run-student-content|20260923-test-run-student-content]]
4. **[ต่ำ] ไม่มีหน้ารวมรายการ "สถานที่โปรด" (My Bookmarks) (TC-008, รอบ 1)** — มีแค่ toggle ต่อหน้า
   รายละเอียดเรื่องราวแต่ละหน้า ไม่มีหน้าอื่นอ่าน collection `Bookmarks` เลย ไม่ตรงกับเจตนาเต็มรูปแบบของ
   AC BL-012 "ให้ดูภายหลังได้" แม้กลไกหลัก (บันทึก/ยกเลิก) จะทำงานถูกต้อง — ดู
   [[../03-testing/02-test-result/20260923-test-run-tourist|20260923-test-run-tourist]]
5. **[ต่ำ] ข้อความสถานะไม่ตรงกันเมื่อ navigate ตรงไปหน้าอื่นหลังบัญชีชุมชนถูกปฏิเสธ (TC-019, รอบ 3)** —
   navigate ตรงไปหน้าอื่น (เช่น `community-dashboard.html`) นอกเหนือจากหน้า login ระบบบล็อกการเข้าถึง
   ถูกต้อง แต่ข้อความที่แสดงคือของสถานะ "รออนุมัติ" แทนที่จะเป็นข้อความเฉพาะของสถานะ "ไม่อนุมัติ" พร้อม
   เหตุผลแบบที่หน้า login แสดงถูกต้อง — ดู
   [[../03-testing/02-test-result/20260923-test-run-community-registration|20260923-test-run-community-registration]]

**ข้อสังเกตรูปแบบบั๊กซ้ำ**: บั๊กข้อ 2 และ 3 เป็นรูปแบบเดียวกันเป๊ะ (ขาด UI/query ฝั่งผู้ส่งคำขอที่แสดง
ประวัติ+เหตุผลของคำขอที่ถูกปฏิเสธ) เกิดซ้ำ 2 ครั้งในคนละ flow (คำขอแก้ไขคอนเทนต์ชุมชน vs ผลงานนิสิต) —
แนะนำให้ทีมพัฒนาแก้พร้อมกันทั้งสองจุดด้วยรูปแบบเดียวกัน — รอบทดสอบที่ 6 (TC-027–032, นิสิต-สมัครบัญชี)
ตรวจ pattern เดียวกันนี้เป็นพิเศษกับ flow "บัญชีถูกปฏิเสธ" (ต่างจาก "ผลงาน/คำขอถูกปฏิเสธ") และ**ไม่พบ
ปัญหาเดียวกัน** จึงสรุปว่าบั๊กจำกัดอยู่เฉพาะ flow "ผลงาน/คำขอแก้ไขถูกปฏิเสธ" เท่านั้น ไม่ใช่ปัญหาเชิง
สถาปัตยกรรมทั่วทั้งระบบ

**ไม่พบบั๊กด้าน data privacy/security เลยตลอดทั้ง 7 รอบ** — โดยเฉพาะ TC-035 (รอบ 7) ที่ทดสอบเจาะลึก
เรื่องการเข้าถึงข้อมูลผลงานที่ยังไม่อนุมัติแบบ public ไม่พบข้อมูลรั่วไหลเลยแม้แต่นิดเดียว ยืนยันว่า
`firestore.rules` บังคับสิทธิ์ถูกต้องทั้งระดับ query และระดับ document read

อัปเดต [[../03-testing/01-test-plan/test-plan|test-plan]] คอลัมน์สถานะ/หมายเหตุของ TC-033–036 และหัวข้อ
"สรุป" ท้ายไฟล์ให้ตรงกับผลทดสอบจริงครบทั้ง 36 ข้อแล้ว

### 2026-09-23 — แก้บั๊ก TC-022/TC-026 (ฝั่งผู้ยื่นคำขอไม่เห็นเหตุผลตอนถูกปฏิเสธ)

บั๊กทั้ง 2 ตัวที่พบจากรอบทดสอบ 36 ข้อ เป็น pattern เดียวกันเป๊ะ: หน้าฝั่งผู้ยื่นคำขอไม่เคย query
กลับมาอ่านสถานะ/เหตุผลของคำขอที่ถูกปฏิเสธเลย — แก้พร้อมกันทั้งคู่เพราะสาเหตุเหมือนกัน

**ตรวจ `LSH/firestore.rules` ก่อนแก้**: พบว่าสิทธิ์อ่านทั้ง `LSHRequests`
(`allow read: if request.auth != null || resource.data.status == 'อนุมัติ'`) และ
`ContentEditRequests` (`allow read: if request.auth != null`) เปิดกว้างให้ผู้ใช้ที่ login แล้ว
ทุกคนอ่านได้อยู่แล้ว — **ไม่ต้องแก้ rules เลย** ปัญหาอยู่ที่ client-side query ไม่เคยถูกเขียนขึ้นมา
เท่านั้น

**แก้ `docs/02-design/01-prototypes/prototype-v2/community-dashboard.html` (TC-022, BL-023)**:
เดิม `editsQuery` filter เฉพาะ `status == 'รอพิจารณา'` เท่านั้น คำขอที่ถูกปฏิเสธจึงไม่เคยถูกดึงมา
เลย — เอา filter สถานะออก ดึงคำขอทั้งหมดของชุมชนมา หาคำขอล่าสุด (ตาม `createdAt`) ต่อ 1
คอนเทนต์ ถ้าล่าสุดคือ "ไม่อนุมัติ" แสดงป้าย + เหตุผลปฏิเสธต่อแถวคอนเทนต์นั้น พร้อมปุ่ม "ขอแก้ไข
อีกครั้ง"

**แก้ `docs/02-design/01-prototypes/prototype-v2/student-publish.html` (TC-026, BL-018)**: เพิ่ม
ส่วนใหม่ "ผลงานที่เคยส่งของคุณ" ใน `form-view` — query `LSHRequests` ด้วย `requesterId ==` uid
ของนิสิตคนนั้น เรียงตาม `createdAt` ใหม่สุดก่อน แสดงสถานะ (รอพิจารณา/อนุมัติ/ไม่อนุมัติ) + เหตุผล
ไม่อนุมัติถ้ามี เรียกตอน login สำเร็จคู่กับ `loadCommunityOptions()` เดิม เพิ่ม `escapeHtml()`
helper ที่ไฟล์นี้ไม่เคยมีมาก่อน (ป้องกัน XSS จากข้อความที่ผู้ใช้กรอกเอง เช่น เหตุผลปฏิเสธ/ชื่อ
ผลงาน)

**ทดสอบ**: serve `prototype-v2` local (`http-server`) ชี้ Firestore project `lsh-nammon` จริง —
login ด้วยบัญชีชุมชนจริงที่มีคำขอถูกปฏิเสธค้างจาก TC-022 (`qa-community-approve-lsh@example.com`)
เห็นเหตุผลปฏิเสธจริงที่เคยกรอกไว้ตอนทดสอบ TC-022 ถูกต้องครบถ้วน — login ด้วยบัญชีนิสิตจริง (u001)
ที่มีผลงานถูกปฏิเสธค้างจาก TC-026 เห็นรายการผลงานครบ 4 รายการพร้อมสถานะ/เหตุผลถูกต้องทุกแถว
(รวมถึงยืนยันว่า dropdown เลือกชุมชนของ BL-013 ยังทำงานถูกต้องอยู่) ไม่มี JS error

**อัปเดตเอกสาร**: [[../01-requirements/03-task/product-backlog|product-backlog]] (BL-018, BL-023 —
เพิ่มหมายเหตุอธิบายบั๊ก+วิธีแก้ พร้อมแก้หมายเหตุเดิมของ BL-018 ที่เข้าใจผิดว่า `rejected-view`
ครอบคลุมกรณีนี้อยู่แล้ว — จริงๆ เป็นข้อความคนละเรื่องคือบัญชีถูกปฏิเสธ ไม่ใช่ผลงานถูกปฏิเสธ),
[[../03-testing/01-test-plan/test-plan|test-plan]] (TC-022, TC-026 status → "แก้ไขแล้ว" + หัวข้อ
"สรุป" ปรับตัวเลขใหม่)

ยังไม่ deploy — รอ commit + `firebase deploy --only hosting` (ไม่ต้อง deploy rules เพราะไม่ได้แก้)

### 20260923 — ผลทดสอบรอบ Regression เต็มรูปแบบ (ทดสอบซ้ำทุก TC ในรอบเดียว)

ตามคำขอผู้ใช้ให้ "รันเทสต์ทั้งหมดอีกรอบ" แบบ regression เต็มรูปแบบ — ทดสอบซ้ำผ่าน browser จริง
(Playwright MCP) บน production ครบทั้ง 37 TC (TC-001–TC-036 + TC-M-001) ในรอบเดียว ไม่ใช่แค่แถวที่
"ยังไม่ทดสอบ" — เพื่อยืนยันว่าแถวที่เคย "ผ่าน" จาก 7 รอบย่อยเช้าวันเดียวกันยังทำงานถูกต้องอยู่จริง

**สรุปผล**: ผ่าน 25, ไม่ผ่าน (พบบั๊ก) 1, บล็อก 9, ข้าม 2 (รวม 37) — ดูรายละเอียดเต็มทุก TC ที่
[[../03-testing/02-test-result/20260923-test-run-regression-full|20260923-test-run-regression-full]]

**ยืนยันซ้ำสำเร็จ**: บั๊กทั้ง 2 ตัวที่แก้ไปในรอบก่อนหน้า (TC-022, TC-026 — ฝั่งผู้ยื่นคำขอไม่เห็น
เหตุผลตอนถูกปฏิเสธ) ยังคงทำงานถูกต้องบน production ไม่ regression กลับไปเป็นบั๊กเดิม

**พบ regression ใหม่ 1 จุด — ความรุนแรงสูง (TC-006 / BL-010)**: `CLAUDE.md` หัวข้อ "ตำแหน่งบนแผนที่ —
BL-010" (อัปเดตของรอบนี้เอง) อ้างว่าเพิ่ม UI ปักหมุดจริงด้วย Leaflet+OpenStreetMap ใน
`student-publish.html`/`community-create-content.html` แล้ว ทดสอบ end-to-end ผ่านครบ และ "ปิดสถานะ
BL-010 เป็นเสร็จแล้ว" — ทดสอบจริงบน production พบว่า**ไม่มี UI ปักหมุด/แผนที่ปรากฏเลยทั้ง 2 หน้า**
(`browser_find` หาคำว่า "ปักหมุด"/"แผนที่"/"ตำแหน่ง" ไม่พบ, ไม่มี network request ไปยัง
`unpkg.com/leaflet` เลย) มีแค่ workaround ลิงก์ Google Maps ค้นหาด้วยชื่อชุมชนแบบเดิมก่อน BL-010 —
ข้อสันนิษฐาน (ไม่ยืนยัน): การทดสอบ "จริง" ที่ CLAUDE.md อ้างอาจทำผ่าน local dev server แล้วไม่เคย
`firebase deploy --only hosting` ขึ้น production จริง — **แนะนำทีมพัฒนาตรวจสอบการ deploy และแก้ไข
สถานะเอกสารให้ตรงกับพฤติกรรมจริงก่อนปิด BL-010 ว่าเสร็จสมบูรณ์**

**ข้อสังเกตอื่น (ไม่แน่ใจ ต้องตรวจเพิ่ม, ความรุนแรงต่ำ)**: ปุ่ม "ส่งขออนุมัติเพื่อเผยแพร่" ที่
`student-publish.html` navigate ออกจากหน้าไปที่ `admin-review-student-work.html` โดยไม่คาดคิด
เกิดขึ้นซ้ำ 2 ครั้งกับ 2 บัญชีต่างกัน — ข้อมูลยังบันทึกสำเร็จถูกต้อง 100% ไม่มีข้อมูลสูญหาย มีความ
เป็นไปได้ว่าเป็น artifact ของเครื่องมือทดสอบมากกว่าบั๊กจริง จึงไม่ฟันธง บันทึกไว้ให้ตรวจสอบเพิ่ม

**ข้อจำกัดของรอบนี้**: TC-001/TC-002 (เคย "ผ่าน" รอบก่อน) ทดสอบซ้ำไม่ได้เพราะ browser profile ของ
Playwright MCP เป็นแบบ persistent มี Consent state ค้างจาก session ก่อนหน้าในวันเดียวกัน ไม่มี
เครื่องมือเคลียร์ storage ให้ใช้โดยไม่ผิดกฎห้าม JS injection — มาร์กเป็นบล็อกชั่วคราว ไม่ใช่บั๊ก
แนะนำรอบถัดไปให้ใช้ browser profile ใหม่

อัปเดต [[../03-testing/01-test-plan/test-plan|test-plan]] คอลัมน์สถานะ/หมายเหตุครบทั้ง 37 แถว และ
หัวข้อ "สรุป" ท้ายไฟล์ให้ตรงกับผลทดสอบ regression รอบนี้ (TC-006 เปลี่ยนจาก "ผ่าน" เป็น "ไม่ผ่าน")
