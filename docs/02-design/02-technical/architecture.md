...
| `CommunityContent` (`title`, `bodyTh`, `caption`, `communityId`, `communityName` denormalized, `status`: `ฉบับร่าง`/`เผยแพร่แล้ว`, `createdAt`, `updatedAt`) *(เพิ่ม 2026-09-23 โดย prototype-v2 — แทนที่ mockup `localStorage` เดิมใน prototype-v3)* | `Content` (BL-006) | ต่างจาก conceptual ตรงที่ `communityId`/`communityName` denormalized (ตาม convention เดียวกับ `LSHRequests.requesterId/Name`) แทนที่จะเป็น reference อย่างเดียว — ยังไม่มี field ผลลัพธ์จาก AI (แปลภาษา/SEO keyword ฯลฯ) เพราะ AI backend ยังไม่ implement จริงในรอบนี้ (รอ Part 3 — Cloud Functions proxy) |
