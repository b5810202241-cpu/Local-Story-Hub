...
| **AI Content Service** | ปรับภาพ (FR-1.1), คิดแคปชัน (FR-1.2), แปลภาษา (FR-1.3), แนะนำ SEO (FR-1.4), แนะนำวิธีเล่าเรื่อง (FR-1.5) — ทำงานแบบ **Synchronous** (ดู Decision Log) — **implement จริงเป็น Cloudflare Worker** (`cf-worker/`, เพิ่ม 2026-09-23) ที่ proxy ไปเรียก OpenRouter อีกที (ไม่ใช่ AI model ของตัวเอง) กันไม่ให้ API key หลุดไปฝั่ง Client — ยังไม่ implement ส่วนปรับภาพ (FR-1.1) เพราะไม่มีระบบอัปโหลดภาพจริง (ดู `cf-worker/README.md`) |
...
| `AccessLogs` (`timestamp`, `ip_address`, `user_agent`, `user_account_id` — null ได้, `action`, บวก `expires_at` ที่ไม่มีในเชิงแนวคิด) *(เพิ่ม 2026-09-23 โดย `cf-worker/`)* | `AccessLog` | field ตรงกับเชิงแนวคิดครบ ยกเว้น `expires_at` (= `timestamp` + 90 วัน) ที่เพิ่มเข้ามาเฉพาะให้ Firestore TTL policy อ่าน ไม่ใช่ field ที่ Business Rule กำหนด — เขียนผ่าน Cloudflare Worker ด้วย **service account** (bypass `firestore.rules` ไปเลย ไม่ใช่ ID token ของผู้เรียกแบบ collection อื่น) เพราะต้อง log ได้แม้ผู้เข้าชมไม่ login เลย — อ่านได้เฉพาะ role=`teacher` (Data Controller) |
...
- **2026-09-23** — AI Content Service (component เชิงแนวคิด) implement จริงเป็น **Cloudflare Worker** ไม่ใช่ Firebase Cloud Functions ตามแผนเดิม — เหตุผล: ผู้ใช้ไม่ต้องการผูกบัตรเครดิตกับ Firebase (Cloud Functions บังคับอัปเกรดเป็นแผน Blaze) ส่วน Cloudflare Workers free tier ใช้ได้โดยไม่ต้องผูกบัตร — ไม่กระทบ conceptual design ของ component นี้ (ยัง Synchronous เหมือนเดิมตาม decision 2026-08-28) เป็นแค่ทางเลือก hosting ของ implementation เท่านั้น
